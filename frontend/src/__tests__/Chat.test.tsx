...
describe('Chat Component', () => {
  beforeEach(() => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ answer: 'This is a mock response from the server.' })
    })
  ...
  it('renders the initial greeting message', () => {
    render(<Chat />)
    expect(screen.getByText(/Hello! I am the Veritas chatbot/i)).toBeInTheDocument()
  })
  ...
  it('submits a message and displays it in the chat', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('This is a mock response from the server.')).toBeInTheDocument()
    })
    
    // fetch should have been called
    expect(mockFetch).toHaveBeenCalledTimes(1)
    expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/chat/query', expect.any(Object))
  })
  ...
  it('displays an error message when the API fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sorry, I am having trouble connecting to the server/i)).toBeInTheDocument()
    })
  })
})
